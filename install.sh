...
echo ""
mkdir -p "$INSTALL_DIR"
cp "$SERVER_DIR/$BINARY_NAME" "$INSTALL_DIR/$BINARY_NAME"
...
source=$SERVER_DIR
EOF
echo -e "${GREEN}  [COPIED] Binary to $INSTALL_DIR/$BINARY_NAME (commit $GIT_COMMIT)${NC}"
...
if ! command -v claude >/dev/null 2>&1; then
    ...
    echo "    claude mcp add -s user -t stdio -- slack $INSTALL_DIR/$BINARY_NAME"
    echo ""
...
CURRENT_PATH=$(claude mcp list 2>/dev/null | grep "^slack:" | awk '{print $2}' || true)

if [ "$CURRENT_PATH" = "$TARGET_PATH" ]; then
    echo -e "${YELLOW}  [SKIP] slack already registered with correct path${NC}"
    echo ""
    echo -e "${GREEN}Installation complete!${NC}"
    echo ""
    exit 0
...
claude mcp remove -s user slack 2>/dev/null || true
